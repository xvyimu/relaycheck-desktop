...
function SitesPanelBase({ sites, onRefresh, intent, onNavigate, dialogEpoch = 0 }: SitesPanelProps) {
  ...
  const summary = useMemo(
    () => ({
      ...
      healthy: filteredSites.filter((site) => ["healthy", "ok", "success"].includes(site.healthStatus.toLowerCase()))
        .length,
  ...
  return (
    <section className="sites-panel">
      ...
      <div className="segmented" role="group" aria-label="视图">
        <button type="button" className={subview === "sites" ? "active" : ""} onClick={() => setSubview("sites")}>
          按站点
        </button>
        <button type="button" className={subview === "accounts" ? "active" : ""} onClick={() => setSubview("accounts")}>
          全部账号
        </button>
      </div>
            ...
            <>
              ...
              <div className="site-grid">
                ...
                {!filteredSites.length ? (
                  <Empty title="暂无上游站点" description="请先导入 NewAPI 或 OneAPI 渠道，再在此探测站点能力。" />
                ) : null}
              </div>
            </>
          )}
                  ...
                  <section className="detail-card">
                    ...
                    <div className="detail-metrics site-login-metrics">
                      <div>
                        <span>来源</span>
                        <strong>
                          {loginDiscoverySourceLabel(detailDiscovery?.source || detail.site.loginUrlSource)}
                        </strong>
                      </div>
                      <div>
                        <span>置信度</span>
                        <strong>
                          {formatConfidence(detailDiscovery?.confidence ?? detail.site.loginUrlConfidence)}
                        </strong>
                      </div>
                    </div>
