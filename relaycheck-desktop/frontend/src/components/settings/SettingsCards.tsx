    ...
    <article className="card settings-about-card">
      <div className="section-heading">
        <div>
          <strong>关于 / 版本</strong>
          <span>
            {status.productName} &middot; {status.productVersion}
          </span>
        </div>
        <span className="status-pill success">
          <StatusLabel level="success" label="正式版" />
        </span>
      </div>
      <div className="detail-list">
        <div>
          <span>显示名</span>
          <strong>{status.productName}</strong>
        </div>
        <div>
          <span>版本</span>
          <strong>{status.productVersion}</strong>
        </div>
        <div>
          <span>构建时间</span>
          <strong>{formatBuildTime(status.buildTime)}</strong>
        </div>
        <div>
          <span>绑定地址</span>
          <strong>
        ...
        <div>
          ...
          <strong>
            {scheduler
              ? `${scheduler.jobs.length} 个任务 &middot; ${schedulerStatusLabel(checkinJob?.status || "idle")}`
              : "读取中"}
          </strong>
        </div>
        <div>
          <span>上次自检</span>
          <strong>
            {status.lastDiagnostics
              ? `${diagnosticLevelLabel(status.lastDiagnostics.overall)} &middot; ${status.lastDiagnostics.itemCount} 项 &middot; ${formatTime(status.lastDiagnostics.generatedAt)}`
              : "未生成"}
          </strong>
        </div>
      </div>
  ...
  return (
    <article className="card settings-version-check-card">
      ...
      {versionCheckResult ? (
        <div className="detail-list" style={{ marginTop: 8 }}>
          ...
          {versionCheckResult.latestVersion ? (
            <div>
              <span>最新版本</span>
              <strong>{versionCheckResult.latestVersion}</strong>
            </div>
          ) : null}
          <div>
            <span>状态</span>
      ...
      <div className="problem-hint detail-hint">
        配置版本清单 URL 后，可检查远程是否有新版本。清单格式:{" "}
        {'{ "version": "v1.1", "releaseUrl": "...", "releaseNotes": "..." }'}
      </div>
    </article>
  );
}
      ...
      {portCheckResult ? (
        <div className="detail-list">
          ...
          <div>
            <span>状态</span>
            <strong>
              {portCheckResult.available ? "可用（未被占用）" : portCheckResult.inUse ? "已被占用" : "检测失败"}
            </strong>
          </div>
          {portCheckResult.error ? (
            <div>
              <span>详情</span>
              <strong>{portCheckResult.error}</strong>
            </div>
          ) : null}
        </div>
      ) : null}
  ...
  return (
    <article className="card settings-path-card">
      ...
      <div className="detail-list">
        <div>
          <span>数据库</span>
          <strong>{status.databasePath}</strong>
        </div>
        <div>
          <span>备份目录</span>
        ...
        <div>
          <span>代理</span>
          <strong>{status.networkProxy?.enabled ? status.networkProxy.urlMasked : "未启用"}</strong>
        </div>
      </div>
  ...
  return (
    <article className="card settings-help-card">
      ...
      <div className="detail-list">
        <div>
          <span>使用说明</span>
          <strong>relaycheck-desktop/README.md</strong>
        </div>
        <div>
          <span>总清单</span>
          <strong>relaycheck-desktop/PROMPT_CHECKLIST.md</strong>
        </div>
        <div>
          <span>设计规则</span>
          <strong>relaycheck-desktop/DESIGN_SYSTEM.md</strong>
        </div>
        <div>
          <span>接力说明</span>
          <strong>relaycheck-desktop/AGENT_HANDOFF.md</strong>
        </div>
      </div>
      {showHelpGuide ? (
        <div className="detail-stack">
          <div className="problem-hint detail-hint">
            新手路径：先去"本机扫描"导入 NewAPI，再到"账号"补授权或 API Key，最后在"签到"和"余额"验证一次。
          </div>
  ...
  return (
    <article className="card settings-legend-card">
      ...
      <div className="detail-list">
        <div>
          <span>Key 有效</span>
          <strong>已读取 /v1/models 且密钥可用</strong>
        </div>
        <div>
          <span>模型可用</span>
          <strong>最小 chat completion 测试通过</strong>
        </div>
        <div>
          <span>raw_json</span>
          <strong>来自 NewAPI 渠道原始配置的回退识别</strong>
        </div>
        <div>
          <span>live</span>
          <strong>使用渠道 Key 实时请求上游模型列表</strong>
        </div>
      </div>
    </article>
  );
    ...
    <article className="card settings-sync-card">
      <div className="section-heading">
        ...
        <span className={"status-pill " + (syncSchedule.enabled ? "success" : "neutral")}>
          <StatusLabel
            level={syncSchedule.enabled ? "enabled" : "disabled"}
            label={syncSchedule.enabled ? "已启用" : "未启用"}
          />
        </span>
      </div>
      <div className="proxy-toggle-row">
        ...
        <label className="check">
          <input
            type="checkbox"
            checked={syncSchedule.runOnStartup}
            onChange={(event) => onPatch({ runOnStartup: event.target.checked })}
          />
          启动后同步一次
        </label>
      </div>
      <div className="proxy-form-grid">
        <label className="field">
          ...
          <input
            ...
            min={5}
            max={1440}
            value={syncSchedule.intervalMinutes}
            onChange={(event) => onPatch({ intervalMinutes: Math.max(5, Number(event.target.value) || 30) })}
          />
        </label>
        <label className="field">
          <span>同步模式</span>
      ...
      <div className="problem-hint detail-hint">
        后台同步默认不导入渠道 Key、不做重探测，只更新渠道结构和源端移除状态；失败才发重要通知。
      </div>
    ...
    <article className="card settings-sync-card">
      <div className="section-heading">
        ...
        <span className={"status-pill " + (channelHealthSchedule.enabled ? "success" : "neutral")}>
          <StatusLabel
            level={channelHealthSchedule.enabled ? "enabled" : "disabled"}
            label={channelHealthSchedule.enabled ? "已启用" : "未启用"}
          />
        </span>
      </div>
      <div className="proxy-toggle-row">
        <label className="check">
          <input
            type="checkbox"
            checked={channelHealthSchedule.enabled}
            onChange={(event) => onPatch({ enabled: event.target.checked })}
          />
          启用自动探测
        </label>
        <label className="check">
          <input
            type="checkbox"
            checked={channelHealthSchedule.runOnStartup}
            onChange={(event) => onPatch({ runOnStartup: event.target.checked })}
          />
          启动后立即探测
        </label>
        <label className="check">
          <input
            type="checkbox"
            checked={channelHealthSchedule.onlyRisky}
            onChange={(event) => onPatch({ onlyRisky: event.target.checked })}
          />
          只探测风险站点
        </label>
      </div>
      <div className="proxy-form-grid">
        <label className="field">
          ...
          <input
            ...
            min={5}
            max={1440}
            value={channelHealthSchedule.intervalMinutes}
            onChange={(event) => onPatch({ intervalMinutes: Math.max(5, Number(event.target.value) || 60) })}
          />
        </label>
        <label className="field">
          <span>单次站点上限</span>
          <input
            ...
            min={1}
            max={50}
            value={channelHealthSchedule.limit}
            onChange={(event) => onPatch({ limit: Math.min(50, Math.max(1, Number(event.target.value) || 20)) })}
          />
        </label>
      </div>
      <div className="problem-hint detail-hint">
        调度器会复用渠道页的“探测健康”流程，发现站点不可达、模型同步失败或 Key 状态异常时记录预警。
      </div>
      <div className="proxy-actions">
        <button disabled={busy || !canSave} onClick={onSave}>
          {busy ? "保存中…" : "保存健康探测计划"}
        </button>
        <Button variant="ghost" disabled={busy} onClick={() => onPatch(defaultConfig)}>
          恢复默认
        </Button>
      </div>
    </article>
...
export function SettingsSchedulerCard({
  scheduler,
  busy,
  onRefresh,
}: {
        ...
        {[
          ...
          <article className={"scheduler-job " + (job?.status || "idle")} key={key}>
            <div>
              ...
              <strong>
                <StatusLabel level={job?.status || "idle"} label={schedulerStatusLabel(job?.status || "idle")} />
              </strong>
            </div>
        ...
        {auditLogs.slice(0, 12).map((item) => (
          <article className={"detail-row audit-row " + item.level} key={item.id}>
            <div>
              ...
              <span>
                {item.summary} {"·"} {formatTime(item.createdAt)}
              </span>
            </div>
            <b>
              <StatusLabel level={item.level} label={auditLevelLabel(item.level)} />
            </b>
          </article>
        ))}
        {!auditLogs.length ? (
          <EmptyState title="暂无审计记录" description="登录、设置、备份、账号和站点维护会在这里留下只读记录。" />
        ) : null}
      </div>
    </article>
  );
  ...
  return (
    <article className="card">
      <div className="section-heading">
        ...
        <button disabled={busy || !settings.length} onClick={onSave}>
          {busy ? "保存中…" : "保存设置"}
        </button>
      </div>
      <div className="settings-list">
        {settings.map((setting, index) => (
          <label className="settings-editor" key={setting.key}>
            <span>
              {setting.key} {"·"} 更新于 {formatTime(setting.updatedAt)}
            </span>
            <textarea
