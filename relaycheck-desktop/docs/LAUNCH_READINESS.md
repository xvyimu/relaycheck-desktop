...
- The application has no admin login. Shared-host deployments should set `RELAYCHECK_REQUIRE_TOKEN=1` before launch and protect `data\session-token.txt`.
...
- Decide whether the target host requires `RELAYCHECK_REQUIRE_TOKEN=1`; record the chosen mode without recording the token.
