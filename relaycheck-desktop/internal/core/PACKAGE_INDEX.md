...
| `account_cleanup_service.go` | `AccountCleanupService`: unsupported-checkin matching, bounded five-minute one-time previews, transactional candidate revalidation, related log/snapshot deletion, and read-cache invalidation. `accounts.go` keeps the two-step handler plus audit/notification wrappers. |
