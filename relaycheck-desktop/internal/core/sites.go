	...
	return cachedRead(a, "upstream-sites-list", shortReadCacheTTL, func() ([]UpstreamSite, error) {
		...
		for i, s := range siteItems {
			items[i] = UpstreamSite{
				...
				BaseURL:             s.BaseURL,
				LoginURL:            s.LoginURL,
				LoginURLSource:      s.LoginURLSource,
				LoginURLConfidence:  s.LoginURLConfidence,
				LoginDiscoveryJSON:  s.LoginDiscoveryJSON,
				LoginDiscovery:      loginDiscoveryFromSites(s.LoginDiscovery),
				Kind:                s.Kind,
...
func (a *App) detectUpstreamSite(w http.ResponseWriter, r *http.Request, id string) {
	...
	writeJSON(w, http.StatusOK, UpstreamDetection{
		...
		HomepageURL:         detection.HomepageURL,
		LoginURL:            detection.LoginURL,
		LoginDiscovery:      loginDiscoveryFromSites(detection.LoginDiscovery),
		Kind:                detection.Kind,
		HealthStatus:        detection.HealthStatus,
		DetectionConfidence: detection.DetectionConfidence,
		SupportsCheckin:     detection.SupportsCheckin,
