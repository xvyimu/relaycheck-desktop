...
func (a *App) ensureUpstreamSiteForChannel(ctx context.Context, channelID string, name string, rawBaseURL string, kind string, detection *UpstreamDetection) (string, bool, error) {
	...
	if detection != nil && !isManagedRelayKind(kind) {
		return "", false, nil
	}
	...
	if err == nil {
		_, err = a.db.ExecContext(ctx, `
			...
			SET channel_id=CASE WHEN COALESCE(channel_id,'')='' THEN ? ELSE channel_id END,
			    ...
			    updated_at=?
			WHERE id=?
		`, channelID, name, homepageURL, loginURL, kind, confidence, detectionJSON, detectionJSON, healthStatus, healthStatus, boolInt(supportsCheckin), boolInt(supportsBalance), boolInt(supportsModels), boolInt(supportsPricing), now(), now(), siteID)
	...
	_, err = a.db.ExecContext(ctx, `
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, siteID, channelID, name, homepageURL, baseURL, loginURL, kind, confidence, healthStatus, boolInt(supportsCheckin), boolInt(supportsBalance), boolInt(supportsModels), boolInt(supportsPricing), detectionJSON, now(), now(), now())
